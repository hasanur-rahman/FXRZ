...
void scan(const char *in_file1_path, 
	...
	if (csv_fin1.is_open() && csv_fin1.good()) {
		while(std::getline(csv_fin1, csv_buffer)) {
			...
			sprintf(command_buffer, "mgardx %s 0 %f 3 1 3 849 849 235 > %s 2>&1", file_name,  ori_err, output_string1.c_str());
			...
			sprintf(command_buffer, "mgardx %s 0 %f 3 1 3 849 849 235 > %s 2>&1", file_name,  pred_err, output_string1.c_str());
