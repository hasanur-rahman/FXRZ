...
void scan(const char *in_file1_path, 
	...
	char *output_buffer = (char* )malloc(300 * sizeof(char));       
        sprintf(output_buffer, "./fpzip-compression-info-ori.txt");
...
;
			...
			sprintf(command_buffer, "fpzip -i %s -o %s.fpzip.out -t float -p %i -3 512 512 512 > %s 2>&1", file_name, file_name, (int)round(ori_err), output_string1.c_str());
			...
			cur_file.close();
                        ...
                        sprintf(command_buffer, "fpzip -i %s -o %s.fpzip.out -t float -p %i -3 512 512 512 > %s 2>&1", file_name, file_name, (int)round(pred_err), output_string1.c_str());
                        cur_command = ""; cur_command += command_buffer;
                        system(cur_command.c_str());
			cur_file.open(output_string1.c_str(), std::ios::in);	
			if(cur_file.is_open()) {
			...
			printf("Target compression ratio: %.4f, Real compression ratio from predicted error bound : %.4f\n", cr1, cr2);
                        csv_fout << (int)round(ori_err) << ", " << (int)round(pred_err) << ", "  << cr1 << ", " << cr2 << ", " << estimationError << "\n";			
	...
	system("rm fpzip-compression-info-ori.txt");
