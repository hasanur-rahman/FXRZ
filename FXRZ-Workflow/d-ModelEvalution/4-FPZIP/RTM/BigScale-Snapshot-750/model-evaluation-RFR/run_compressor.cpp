...
;
			...
			sprintf(command_buffer, "fpzip -i %s -o %s.fpzip.out -t float -p %i -3 235 849 849 > %s 2>&1", file_name, file_name, (int)round(ori_err), output_string1.c_str());
			...
			cur_file.close();
                        ...
                        sprintf(command_buffer, "fpzip -i %s -o %s.fpzip.out -t float -p %i -3 235 849 849 > %s 2>&1", file_name, file_name, (int)round(pred_err), output_string1.c_str());
                        cur_command = ""; cur_command += command_buffer;
                        system(cur_command.c_str());
			cur_file.open(output_string1.c_str(), std::ios::in);	
			if(cur_file.is_open()) {
