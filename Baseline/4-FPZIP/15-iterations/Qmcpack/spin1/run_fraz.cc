...
int main(int argc, char *argv[]) {
  ...
  pressio_options_set_string(options, "opt:compressor", "fpzip");
  ...
  pressio_options_set_string(options, "fpzip:metric", "composite");
  ...
  helper_set_data(options, "dist_gridsearch:overlap_percentage", {0.0});
  ...
  uint8_t is_integral_a[] = {1};
  size_t is_integral_size[] = {std::size(is_integral_a)};
  const char* outputs[] = {
    "size:compression_ratio"
  };

  pressio_data* is_integral = pressio_data_new_copy(pressio_uint8_dtype, is_integral_a, std::size(is_integral_size), is_integral_size);
 
  pressio_options_set_data(options, "opt:is_integral", is_integral);
  ...
  helper_set_data(options, "dist_gridsearch:num_bins", {3});
  helper_set_data(options, "opt:lower_bound", {6});
  helper_set_data(options, "opt:upper_bound", {24});
  pressio_options_set_uinteger(options, "opt:max_iterations", 5);
