...
int main(int argc, char *argv[]) {
  ...
  pressio_options_set_string(options, "opt:objective_mode_name", "target"); //target
  pressio_options_set_uinteger(options, "opt:do_decompress", 0);
  pressio_options_set_string(options, "sz:error_bound_mode_str", "vr_rel");
  ...
  helper_set_data(options, "dist_gridsearch:num_bins", {3.0}); //3.0

  helper_set_data(options, "opt:lower_bound", {0.0});
  helper_set_data(options, "opt:upper_bound", {0.00028});
